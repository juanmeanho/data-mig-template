...
export default {
  ...
  methods: {
    ...
    async getIndicators() {
      ...
      data.data.forEach(e => {
        let country = countries.filter(function(d) {
          return d.ISO3166A3 == e.iso3.trim();
        })[0];
        if (country) {
          e.latitude = country.latitude;
          e.longitude = country.longitude;
          e.DescripcionING = country.DescripcionING;
      ...
      this.layer_policies
        ...
        .join(
          ...
          update =>
            update.attr("d", this.path).attr("class", function(d) {
              return d.id + " country-path";
            }),
          exit => exit.remove()
        );
    ...
    drawGeneralMarkers(migi_data) {
      ...
      circles
        .on("mouseover", function(d) {
          ...
          d.tooltip = ref.infoTooltip(ref.tooltipMarker(title));
        })
    ...
    async drawMarkers(migi_data, subject_id, subject_name) {            
      ...
      if (categories.length > 0) {
        ...
        labels = d3
          .map(categories, function(d) {
            if (this.lang == "sp") return d.name_es;
            return d.name_en;
          })
          .keys();
      } else {
        if (migi_data_filtered[0].type == "Boolean") {
          if (this.lang == "sp") labels = ["Si", "No"];
          else labels = ["Yes", "No"];
        } else {
          labels = d3
            .map(migi_data_filtered, function(d) {
              return d.description == undefined || d.description == null
                ? "ND"
                : d.description;
            })
            .keys();
      ...
      circles
        .on("mouseover", function(d) {
          ...
          let links = [];
          let observations = d.observactions;
          this.dataTooltip = ref.tooltipMarker(
            title,
            description,
            links,
            observations
          );
          d.tooltip = ref.infoTooltip(this.dataTooltip);
        })
        .on("mouseout", function(d) {
          d3.selectAll(".tooltip").remove();
          this.dataTooltip = {};
    ...
    mapOnClick(map, layer_policies) {
      if ($("select").val() == 99) {
        layer_policies.classed("invisible", true);
      } else {
